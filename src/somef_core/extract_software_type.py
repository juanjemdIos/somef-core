...
def check_static_websites(path_repo, repo_metadata: Result):
    ...
    for root, dirs, files in os.walk(path_repo):
        for file in files:
            ...
            if file.endswith(constants.code_extensions) or file.endswith(
                    constants.ontology_extensions) or file.lower() in (('bower.json', 'package.json')):
                return False
                    ...
                    try:
                        with open(os.path.join(root, file), "r") as readme_file:
                            ...
                            for sentence in sentences:
                                if re.search(rf'\b{title}\b', sentence, flags=re.IGNORECASE) and re.search(
                                        r'\b(pipeline|workflow)\b', sentence, flags=re.IGNORECASE):
                                    return True
...
def check_name(filename):
    ...
    ".py", ".java", ".cpp", ".c", ".php", ".rb", ".js", ".html", ".css", ".go", ".swift", ".scala", ".pl", ".ipynb")
    if pattern1.search(filename) and pattern2.search(filename):
        ...
        return False
    elif (not pattern1.search(filename) and not pattern2.search(filename)) or (
            not pattern1.search(filename) and pattern2.search(filename)):
        return False
